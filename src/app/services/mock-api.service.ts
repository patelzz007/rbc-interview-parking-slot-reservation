...
export class MockApiService {
  ...
  private readonly _parkingSpaces = signal<ParkingSpace[]>([
    ...MOCK_PARKING_SPACES,
  ]);
  ...
  readonly activeReservationsCount = computed(
    () =>
      this._reservations().filter((r) => r.status === ReservationStatus.ACTIVE)
        .length,
  );
  ...
  getReservation(id: string): Signal<Reservation | undefined> {
    ...
    if (!reservation) {
      return toSignal(
        throwError(() => new Error(`Reservation with id ${id} not found`)).pipe(
  ...
  createReservation(
    ...
    if (!lot) {
      return toSignal(
        throwError(
          () => new Error(`Parking lot with id ${dto.lotId} not found`),
  ...
  updateReservation(
    ...
    if (dto.checkInDateTime || dto.checkOutDateTime) {
      ...
      const checkOut = new Date(
        dto.checkOutDateTime || existing.checkOutDateTime,
      );
      const hours = (checkOut.getTime() - checkIn.getTime()) / (1000 * 60 * 60);
      const lot = this._parkingLots().find(
        (l) => l.id === (dto.lotId || existing.lotId),
      );
      if (lot) {
        totalCost = Math.max(0, hours * lot.pricePerHour);
      }
  ...
  getAvailableSpaces(lotId: string): Signal<ParkingSpace[] | undefined> {
    const spaces = this.parkingSpaces().filter(
      (s) => s.lotId === lotId && s.status === SpaceStatus.AVAILABLE,
    );
