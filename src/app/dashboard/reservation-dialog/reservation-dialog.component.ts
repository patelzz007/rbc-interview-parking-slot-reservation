...
export class ReservationDialogComponent implements OnInit, OnDestroy {
  ...
  private setupSearchFilters(): void {
    ...
    // Filter statuses
    this.filteredStatuses$ = combineLatest([
      of(this.statuses),
      this.statusSearchControl.valueChanges.pipe(
        startWith(''),
        debounceTime(200),
        distinctUntilChanged(),
      ),
    ...
    // Filter parking lots
    this.filteredLots$ = combineLatest([
      of(this.parkingLots),
      this.lotSearchControl.valueChanges.pipe(
        startWith(''),
        debounceTime(200),
        distinctUntilChanged(),
      ),
  ...
  private setupSpaceSearchFilter(lotId: string): void {
    ...
    this.filteredSpaces$ = combineLatest([
      of(lotSpaces),
      this.spaceSearchControl.valueChanges.pipe(
        startWith(''),
        debounceTime(200),
        distinctUntilChanged(),
      ),
      ...
      map(([spaces, searchTerm]) => {
        ...
        return spaces.filter(
          (space) =>
            space.spaceNumber.toString().includes(searchTerm) ||
            space.type.toLowerCase().includes(searchTerm) ||
            `Space ${space.spaceNumber}`.toLowerCase().includes(searchTerm),
        );
      }),
    );
  }
